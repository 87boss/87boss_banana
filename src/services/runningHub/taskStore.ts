...
import { submitTask, queryTaskOutputs, cancelTask as cancelTaskApi } from './api';
                        ...
                        try {
                            ...
                            switch (result.code) {
                                case API_CODE.SUCCESS:
                                    ...
                                    // 自动保存到预设目录（如果已配置）
                                    // 自动保存到预设目录（如果已配置）
                                    const autoSaveEnabled = appService.getAutoSaveEnabled();
                                    if (autoSaveEnabled && normalizedResults.length > 0) {
                                        console.log('[Task] Auto-saving results...');
                                        // Process each file
                                        for (let idx = 0; idx < normalizedResults.length; idx++) {
                                            const r = normalizedResults[idx];
                                            const name = `${task.appName || 'output'}_${taskId.slice(0, 8)}_${idx + 1}${getFileExtension(r.fileUrl)}`;

                                            // Call Adapter (Handles both Electron and Browser/Backend)
                                            rhAdapter.saveFile(r.fileUrl, name)
                                                .then(res => {
                                                    if (res.success) console.log(`[Task] Saved: ${res.path || name}`);
                                                    else console.warn(`[Task] Save failed: ${res.error}`);
                                                })
