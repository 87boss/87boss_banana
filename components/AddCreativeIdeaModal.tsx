...
export const AddCreativeIdeaModal: React.FC<AddCreativeIdeaModalProps> = ({ isOpen, onClose, onSave, ideaToEdit, presetImageUrl, presetPrompt, presetAspectRatio, presetResolution }) => {
  ...
  const handleAddBPField = (type: BPFieldType) => {
    ...
    const newField: BPField = {
      ...
      label: type === 'input' ? `變量${count}` : `智能體${count}`,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log('[創意庫] 封面圖已壓縮到512px');
      } catch (e) {
        console.warn('[創意庫] 封面圖壓縮失敗，使用原圖:', e);
      }
  ...
  const modalTitle = ideaToEdit ? "編輯創意" : "新增創意到庫";
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-semibold" style={{ color: theme.colors.textPrimary }}>確認關閉</h3>
                <p className="text-sm mt-0.5" style={{ color: theme.colors.textMuted }}>未保存的內容將丟失</p>
              </div>
            </div>
      ...
      >
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          <div className="w-72 flex-shrink-0 border-r p-4 space-y-3 overflow-y-auto custom-scrollbar" style={{ borderColor: theme.colors.border }}>
            {/* 效果图 */}
            <div>
              <label className="text-[10px] font-medium mb-1.5 block" style={{ color: theme.colors.textMuted }}>效果圖 *</label>
              ...
              {previewUrl ? (
                ...
                <label htmlFor="idea-image-upload" className="flex flex-col items-center justify-center w-full aspect-[3/2] border border-dashed rounded-lg cursor-pointer hover:border-blue-500/50 transition-all" style={{ background: isLight ? 'rgba(0,0,0,0.02)' : 'rgba(255,255,255,0.02)', borderColor: theme.colors.border }}>
                  ...
                  <p className="text-[10px]" style={{ color: theme.colors.textMuted }}>上傳圖片</p>
                </label>
              )}
            </div>
            ...
            {/* 標題 */}
            <div>
              <label className="text-[10px] font-medium mb-1 block" style={{ color: theme.colors.textMuted }}>標題 *</label>
              <input
                type="text"
                value={title}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] font-medium mb-1 block" style={{ color: theme.colors.textMuted }}>寬高比</label>
                <select
              ...
              <div>
                <label className="text-[10px] font-medium mb-1 block" style={{ color: theme.colors.textMuted }}>解析度</label>
                <select
            ...
            {(ideaType === 'bp' || ideaType === 'smartPlus') && (
              <div className="p-2.5 bg-blue-500/5 border border-blue-500/20 rounded-lg space-y-1.5">
                ...
                <label className="flex items-center justify-between cursor-pointer py-0.5">
                  <span className="text-[10px]" style={{ color: theme.colors.textMuted }}>允許查看提示詞</span>
                  <div className="relative">
                    <input
                      type="checkbox"
                ...
                <label className={`flex items-center justify-between cursor-pointer py-0.5 ${!allowViewPrompt ? 'opacity-40' : ''}`}>
                  <span className="text-[10px]" style={{ color: theme.colors.textMuted }}>允許編輯提示詞</span>
                  <div className="relative">
                    <input
                      type="checkbox"
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <div className="px-5 py-3 border-b flex items-center justify-between flex-shrink-0" style={{ borderColor: theme.colors.border }}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium" style={{ color: theme.colors.textPrimary }}>
                  {ideaType === 'standard' && "提示詞編輯"}
                  {ideaType === 'smartPlus' && "基礎場景"}
                  {ideaType === 'bp' && "BP 編排模板"}
                </span>
              </div>
              {/* BP模式下显示可点击的变量标签 */}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {bpFields.map(f => (
                    <button
                      ...
                      title={`點擊插入 ${f.type === 'agent' ? `{${f.name}}` : `/${f.name}`}`}
                    >
            ...
            <div className="flex-1 p-5 flex flex-col min-h-0">
              <textarea
                ...
                className={`flex-1 w-full p-4 border rounded-xl resize-none text-sm leading-relaxed font-mono outline-none transition-all ${ideaType === 'bp'
                  ? 'focus:ring-2'
              ...
              <div className="px-5 pb-4 flex-shrink-0">
                ...
                >
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-xs font-medium" style={{ color: '#eed16d' }}>BP 變量配置</span>
                    <div className="flex gap-2">
                      ...
                      >
                        <PlusCircleIcon className="w-3 h-3" /> 手動變量
                      </button>
                      <button
                      ...
                      >
                        <LightbulbIcon className="w-3 h-3" /> 智能體變量
                      </button>
                    </div>
                  </div>
                  <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                    {bpFields.map(v => (
                      <div key={v.id} className={`p-2.5 rounded-lg border ${v.type === 'agent' ? 'bg-blue-900/20 border-blue-700/50' : 'bg-blue-900/10 border-blue-800/30'
                        }`}>
                        <div className="flex gap-2 items-center">
                          <div className="relative flex items-center shrink-0">
                            ...
                            <input
                              ...
                              className={`w-20 pl-4 pr-2 py-1 bg-gray-800 border rounded text-[11px] text-white outline-none font-mono ${v.type === 'agent' ? 'border-blue-600 focus:border-blue-400' : 'border-blue-700 focus:border-blue-500'
                                }`}
                              placeholder="name"
                            />
            ...
            {ideaType === 'smartPlus' && (
              <div className="px-5 pb-4 flex-shrink-0">
                <div className="p-4 bg-blue-500/5 border border-blue-700/30 rounded-xl">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-xs font-medium text-blue-400">Smart+ 導演模式配置</span>
                    <button
                    ...
                    >
                      <PlusCircleIcon className="w-3 h-3" /> 添加組件
                    </button>
                  </div>
