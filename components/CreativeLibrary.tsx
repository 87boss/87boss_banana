...
export const CreativeLibrary: React.FC<CreativeLibraryProps> = ({ ideas, onBack, onAdd, onDelete, onDeleteMultiple, onEdit, onUse, onExport, onImport, onImportById, onReorder, onToggleFavorite, isImporting, isImportingById }) => {
  ...
  const handleExportSelected = async () => {
    ...
    try {
      ...
      alert(`成功導出 ${selectedIds.size} 個創意`);
    } catch (e) {
  ...
  const handleDeleteSelected = () => {
    ...
    if (window.confirm(`確認刪除選中的 ${selectedIds.size} 個創意？`)) {
      if (onDeleteMultiple) {
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: theme.colors.textMuted }}>管理和使用您的創意靈感</p>
        </div>
        <div className="flex items-center gap-2">
          <a
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 font-semibold rounded-lg text-xs transition-all duration-200 hover:opacity-80"
            style={{
      ...
      <div className="flex-shrink-0 flex items-center justify-between gap-3 py-3">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 font-semibold rounded-lg text-xs transition-all duration-200 ${isMultiSelectMode ? 'bg-purple-500 text-white' : ''
            }`}
          style={{
        ...
        >
          {filterButtons.map(({ key, label }) => (
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-all duration-200 ${filter === key
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium" style={{ color: theme.colors.textPrimary }}>
              已選中 {selectedIds.size} / {filteredIdeas.length} 項
            </span>
            <button
              onClick={selectAll}
        ...
        {filteredIdeas.length > 0 ? (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-3">
            {filteredIdeas.map(idea => {
              ...
              return (
                <div
                  key={idea.id}
                  className={`group relative rounded-xl overflow-hidden cursor-pointer aspect-square transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 ${isSelected ? 'ring-2 ring-purple-500 ring-offset-2' : ''
                    }`}
                  style={{
                    background: theme.colors.bgSecondary,
                ...
                >
                  ...
                  {isMultiSelectMode && (
                    <div
                      className={`absolute top-2 left-2 w-5 h-5 rounded-md border-2 flex items-center justify-center z-10 transition-all duration-200 ${isSelected
                    ...
                    <div className="absolute top-1.5 right-1.5 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200" style={{ cursor: 'default' }}>
                      ...
                      {onToggleFavorite && (
                        <button
                          ...
                          className={`p-1 rounded-full backdrop-blur-sm transition-all duration-200 ${idea.isFavorite
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (window.confirm(`確認刪除 "${idea.title}"?`)) {
                            onDelete(idea.id);
                          }
                        }}
          ...
          <div className="text-center flex flex-col items-center justify-center h-full">
            ...
            <h2 className="text-lg font-semibold" style={{ color: theme.colors.textSecondary }}>
              {searchTerm || filter !== 'all' ? '未找到創意' : '創意庫是空的'}
            </h2>
            <p className="mt-1 text-sm" style={{ color: theme.colors.textMuted }}>
              {searchTerm || filter !== 'all' ? '請嘗試其他關鍵詞或篩選條件' : '點擊 "新增" 來添加您的第一個靈感！'}
            </p>
          </div>
        )}
      </main>
