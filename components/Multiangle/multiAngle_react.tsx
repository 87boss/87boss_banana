/**
 * ComfyUI-Easy-Use: 視覺化調節視角功能 - React/TypeScript 版本
 * ============================================================
 * 
 * 這個檔案展示如何用 React + TypeScript 實現相同的視角調節功能
 * 適用於 Node.js + React 專案
 */
...
interface AngleParams {
  ...
  zoom: number;        // 縮放距離 0-10
  addAnglePrompt?: boolean; // 是否新增詳細角度資訊
}
...
export const MultiAngleControl: React.FC<MultiAngleControlProps> = ({
  ...
  return (
    <div style={styles.container}>
      <h3 style={styles.title}>🎬 視覺化視角控制</h3>
      ...
      <div style={styles.sliderGroup}>
        <label style={styles.label}>
          縮放距離 (Zoom): {params.zoom.toFixed(1)}
        </label>
        <input
          type="range"
          min={0}
      ...
      {showPreview && (
        <div style={styles.preview}>
          <div style={styles.previewTitle}>生成的提示詞:</div>
...
// 輸出: "front-right view, high angle, medium close-up (horizontal: 45, vertical: 30, zoom: 7.0)"
...
function App() {
  return (
    <MultiAngleControl
      onChange={(params, result) => {
        console.log('當前引數:', params);
        console.log('生成提示詞:', result.prompt);
      }}
    />
  );
}
...
function MyComponent() {
  ...
  return (
    <div>
      <button onClick={() => setRotate(90)}>設定右側視角</button>
      <p>{result.prompt}</p>
    </div>
  );
}
