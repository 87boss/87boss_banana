...
const funnyErrorMessages = [
  { emoji: "🐧", title: "哎呀，企鵝滑倒了！", subtitle: "讓我們給它一點時間站起來再試" },
  { emoji: "🧙‍♂️", title: "魔法能量不足！", subtitle: "可能需要更多的想像力燃料" },
  { emoji: "🛠️", title: "AI 在摸魚！", subtitle: "我們已經通知它回來工作了" },
  { emoji: "🌌", title: "宇宙射線干擾！", subtitle: "或者只是網絡有點小情緒" },
  { emoji: "🎨", title: "調色板打翻了！", subtitle: "讓我們重新整理一下顏料" },
  { emoji: "🚀", title: "火箭發射失敗！", subtitle: "但我們會再次嘗試升空" },
  { emoji: "🔮", title: "水晶球變得混濁！", subtitle: "請稍等，讓它重新清晰" },
  { emoji: "☕", title: "AI 去喝咖啡了！", subtitle: "它很快就會回來繼續創作" },
];
    ...
    <div className="flex flex-col items-center justify-center gap-5 text-center max-w-md px-6 py-4">
      ...
      <div className="flex gap-1.5 mt-2">
        {messages.map((_, idx) => (
          <div
            key={idx}
            className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === messageIndex ? 'bg-blue-400 scale-125' : 'bg-gray-600'
              }`}
          />
        ))}
      </div>
    ...
    <div className="flex flex-col items-center justify-center max-w-md mx-auto text-center">
      ...
      <div className="p-3 border-l-4 border-gray-500 bg-gray-900/20 rounded-r-lg w-full">
        <p className="text-gray-300 text-xs text-left">
          <span className="font-semibold">技術詳情：</span> {message}
        </p>
      </div>
  ...
  const renderContent = () => {
    switch (status) {
      ...
      case ApiStatus.Success:
        if (content?.imageUrl) {
          const url = normalizeImageUrl(content.imageUrl);
          const isVideo = url.toLowerCase().endsWith('.mp4') || url.toLowerCase().endsWith('.webm');

          return (
            <div className="flex flex-col items-center gap-5">
              {/* 预览图/视频 - 保持实际比例，占屏幕约一半 */}
              <div
                className="relative cursor-pointer rounded-2xl overflow-hidden shadow-2xl hover:scale-[1.01] transition-all duration-300 group bg-black"
                onClick={() => !isVideo && onPreviewClick(content.imageUrl!)}
              >
                {isVideo ? (
                  <video
                    src={url}
