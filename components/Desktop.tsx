...
export const Desktop: React.FC<DesktopProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <a
          href="https://line.me/ti/p/~webber1229"
          target="_blank"
          rel="noopener noreferrer"
          className="px-2 py-0.5 rounded cursor-pointer transition-colors hover:bg-green-500/10 hover:text-green-500 text-center flex items-center justify-center"
          title="點擊加入 LINE 好友"
          style={{
        ...
        <button
          onClick={() => {
            navigator.clipboard.writeText('web5200602').then(() => {
              alert('微信 ID 已複製，正在嘗試開啟微信...');
              window.location.href = 'weixin://';
            });
          }}
          className="px-2 py-0.5 rounded cursor-pointer transition-colors hover:bg-green-500/10 hover:text-green-500"
          title="點擊複製 ID 並開啟微信"
          style={{
            color: theme.colors.textSecondary,
            border: `1px solid ${theme.colors.border}`,
        ...
        <a
          href="https://www.runninghub.ai/?inviteCode=rh-v1302"
          target="_blank"
          rel="noopener noreferrer"
          className="px-2 py-0.5 rounded cursor-pointer transition-colors hover:bg-blue-500/10 hover:text-blue-500 flex items-center justify-center"
          style={{
            color: theme.colors.textSecondary,
            border: `1px solid ${theme.colors.border}`,
          }}
      ...
      {currentItems.map(item => {
        ...
        return (
          <div
            key={item.id}
            className={`absolute select-none cursor-pointer transition-transform ${isDragging && isSelected ? 'z-50' : 'z-10'
              }`}
            style={{
