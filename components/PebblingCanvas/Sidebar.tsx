  ...
  return (
    <>
        ...
        <button 
            ...
            title={isCanvasLoading ? '載入中...' : canvasName}
        >
        ...
        {onManualSave && (
            <button 
                ...
                title={hasUnsavedChanges ? "有未儲存的修改，點選儲存" : "儲存畫布"}
            >
            ...
            >
                ...
                <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-white">畫布管理</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <button 
                ...
                <div className="px-4 py-2 bg-emerald-500/5 border-b border-white/5">
                    <div className="text-[10px] text-zinc-500 mb-1">當前畫布</div>
                    {isEditingName ? (
                        <input
                            type="text"
                            value={editingName}
                        ...
                        >
                            <span className="text-sm text-white font-medium truncate flex-1">
                                {isCanvasLoading ? '載入中...' : canvasName}
                            </span>
                ...
                <div className="max-h-80 overflow-y-auto" onWheel={(e) => e.stopPropagation()}>
                    {canvasList.length === 0 ? (
                        <div className="p-4 text-center text-zinc-500 text-sm">暫無畫布</div>
                    ) : (
                        canvasList
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="text-sm text-zinc-200 truncate flex items-center gap-2">
                                            ...
                                            {canvas.id === currentCanvasId && (
                                                <span className="text-[9px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded-full">當前</span>
                                            )}
                                        </div>
                                        <div className="text-[10px] text-zinc-500 mt-0.5">
                                            {canvas.nodeCount} 個節點 · {new Date(canvas.updatedAt).toLocaleDateString()}
                                        </div>
                                    </div>
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            if (confirm(`確定刪除畫布「${canvas.name}」嗎？`)) {
            ...
            >
                                            ...
                                            <div className="text-[9px] text-zinc-400 leading-relaxed line-clamp-2">
                                                {idea.isBP && idea.bpFields ? (
                                                    <span className="text-zinc-500">
                                                        輸入: {idea.bpFields.map(f => f.label).join(', ')}
                                                    </span>
                                                ) : idea.isWorkflow && idea.workflowNodes ? (
                                                    <span className="text-zinc-500">
                                                        {idea.workflowNodes.length} 個節點
                                                    </span>
                                                ) : (
                                    ...
                                    <div className="absolute left-full top-0 ml-2 w-64 bg-[#1c1c1e] border border-white/10 rounded-xl p-3 shadow-2xl z-50 pointer-events-none animate-in fade-in slide-in-from-left-2 duration-150">
                                        ...
                                        {idea.isBP && idea.bpFields ? (
                                            <div className="space-y-1">
                                                <div className="text-[10px] text-zinc-500">輸入欄位:</div>
                                            ...
                                            <div className="space-y-1">
                                                <div className="text-[10px] text-zinc-500">工作流輸入:</div>
                ...
                {userPresets.length > 0 && (
                    <div className="pt-2 border-t border-white/10">
                        <h3 className="text-[10px] font-bold uppercase text-zinc-500 mb-2 tracking-wider">畫布預設</h3>
                        <div className="space-y-1 max-h-32 overflow-y-auto" onWheel={(e) => e.stopPropagation()}>
                            {userPresets.slice(0, 3).map((preset) => (
                                ...
                                >
                                    ...
                                    <span className="text-[9px] text-zinc-500 ml-2">({preset.nodes.length} 節點)</span>
                                </button>
                            ))}
                        </div>
                    </div>
