  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBalance(`連線失敗: ${response.status}`);
      }
    } catch (e) {
      setBalance('連線失敗: 網路錯誤');
    }
    setIsLoading(false);
  };
      ...
      <div className="bg-[#1a1a24] border border-white/10 rounded-2xl w-full max-w-md mx-4 shadow-2xl">
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/50">配置 AI 服務介面</p>
            </div>
          </div>
          ...
          {activeTab === 'gemini' ? (
            ...
            <>
              <div>
                ...
                <p className="mt-1 text-xs text-white/40">
                  獲取 API Key: <a href="https://ai.t8star.cn/register?aff=64350e39653" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">ai.t8star.cn</a>
                </p>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm text-white/70 mb-2">影象模型</label>
                ...
                <div>
                  <label className="block text-sm text-white/70 mb-2">文字模型</label>
            ...
            <>
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl px-4 py-3 mb-2">
                <p className="text-xs text-yellow-300">ℹ️ Sora 影片生成需要 OpenAI API 訪問許可權，也可以使用第三方代理服務</p>
              </div>
              <div>
              ...
              <div>
                ...
                />
                <p className="mt-1 text-xs text-white/40">支援第三方代理地址，如 T8star 等</p>
              </div>
            </>
          )}
        </div>
        <div className="p-4 border-t border-white/10 flex justify-end gap-3">
          ...
          <button onClick={handleSave} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 text-white font-medium hover:opacity-90 transition-all">
            {saveStatus === 'saved' ? '已儲存 ✓' : saveStatus === 'error' ? '儲存失敗' : '儲存配置'}
          </button>
        </div>
      </div>
    </div>
